...
class FlashAttentionMetadataBuilder(AttentionMetadataBuilder[FlashAttentionMetadata]):
    ...
    def build(
        ...
        if self.dcp_world_size > 1:
            ...
            # After DCP distribution, the maximum number of tokens on this
            # rank is ceil(L / (S * I)) * R * I, where L is max_seq_len,
            # S is the token-split factor (dcp_world_size, or sum(ratios)
            # under uneven DCP), R is this rank's ratio (1 when even), and
            # I is cp_kv_cache_interleave_size.
...
class FlashAttentionImpl(AttentionImpl):
    ...
    def _forward_with_dcp(
        ...
        (dcp_context_out,) = current_workspace_manager().get_simultaneous(
            (
                (n, self.num_heads_across_dcp, self.head_size),
                self._dcp_dtype,
            ),
        )
        ...
        # negligible.
        suffix_query = query_across_dcp if self.cp_uneven else query
        suffix_num_heads = (
            self.num_heads_across_dcp if self.cp_uneven else self.num_heads
        )
        (dcp_query_out,) = current_workspace_manager().get_simultaneous(
            ((query.shape[0], suffix_num_heads, self.head_size), self._dcp_dtype),
        )
