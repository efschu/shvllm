...
def resolve_kv_cache_block_sizes(
    ...
    if len(groups) <= 1:  # Single group: block_size * dcp * pcp
        # (sum(--rank-tp-ratio) instead of dcp * pcp under uneven DCP)
        bs = cache_config.block_size * cp_token_split_factor(dcp, pcp)
        return bs, bs

    if dcp != 1 or pcp != 1:
        ratios = vllm_config.parallel_config.rank_tp_ratio
        uneven_dcp = bool(ratios) and dcp > 1 and len(set(ratios)) > 1 and pcp == 1
        if not uneven_dcp:
            raise ValueError(
    ...
    # Effective per-group token span of one scheduler block: token-split
    ...
    group_block_sizes = [
        ...
        * (
            ...
            else cp_token_split_factor(dcp, pcp)
        )
        for g in groups
    ]
    ...
    if len(kv_cache_groups) == 1 and isinstance(
        ...
        if cp_rank_ratio > 1:
            raise NotImplementedError(
                ...
                "(VLLM_USE_PACKED_HMA_KV_CACHE / DeepSeek V4)."
            )
...
def get_kv_cache_configs(
    ...
    if uneven_dcp and len(kv_cache_configs) > 1:
        # Self-calibration hint: the token vector was derived from an
