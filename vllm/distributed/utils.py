...
def resolve_cp_token_ratios(vllm_config) -> list[int] | None:
    ...
    override = _os.environ.get("VLLM_UNEVEN_TOKEN_VECTOR")
    if override:
        # Self-calibration override (see the imbalance warning in
    ...
    if isinstance(budgets, list) and len(budgets) == len(weights):
        # v4.1: split the KV cache proportionally to each rank's FREE
